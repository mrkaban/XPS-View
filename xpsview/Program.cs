...
namespace xpsview
{
    static class Program
    {
        ...
        static private void WaitWindow()
        {
            ...
            {
                LogFile.Error("Ошибка при запуске потока ожидания:\n", ex);
            }
        }
        ...
        static void Main()
        {
            ...
            try
            {
                if (fileToLoad == "")
                {
                    ...
                    dialog.Filter = Lang.translate("XPS документы|*.xps|Все файлы|*.*", "XPS Dokumente|*.xps|Alle Dateien|*.*", "XPS papiers|*.xps|Tous les fichiers|*.*");
                    dialog.RestoreDirectory = true;
                    dialog.AddExtension = true;
                    dialog.FilterIndex = 0;
                    dialog.Multiselect = false;
                    dialog.Title = Lang.translate("Выберите файл XPS...", "Wählen Sie eine XPS-Datei aus...", "Choisissez un fichier XPS...");
                    dialog.ShowDialog();
            ...
            {
                LogFile.Error("Ошибка при открытии диалога открытия файла:\n", ex);
            }
            ...
            {
                LogFile.Debug("Обнаружена Win 8");
            ...
            if (stopLoading == false)
            {
                ...
                if (run)
                    ...
                    MessageBox.Show(Lang.translate("Документ не может быть открыт!", "Das Dokument konnte nicht geöffnet werden!", "Le document ne peut pas être ouvert!"), "XPS Viewer", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1, MessageBoxOptions.DefaultDesktopOnly);
